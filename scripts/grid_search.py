        ...
        for offset in offset_list:
            for step_size in step_size_list:
                for window_size in window_size_list:
                    ...
                    perform_cca(ds, sig1, sig2, target_path_GR+addition, DIM, 'clip', entropy, 'all', offset, step_size, window_size)
                    perform_cca(unlikely_ds, sig1, sig2, unlikely_path_GR+addition, DIM, 'clip', entropy, 'unlikely', offset, step_size, window_size)
                    perform_cca(possibly_ds, sig1, sig2, possibly_path_GR+addition, DIM, 'clip', entropy, 'possibly', offset, step_size, window_size)
                    perform_cca(probably_ds, sig1, sig2, probably_path_GR+addition, DIM, 'clip', entropy, 'probably', offset, step_size, window_size)
